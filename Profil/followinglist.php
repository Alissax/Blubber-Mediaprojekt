    ...
    <div id="about">
        <div class="container16">
            <div class="column16">
                ...
                echo "<br>";
                $db = new PDO($dsn, $dbuser, $dbpass);
                $sql = "SELECT * FROM follows INNER JOIN users ON follows.friend_id=users.user_id WHERE follows.user_id = :festgelegteUserID";
                $query = $db->prepare($sql);
                $query->bindParam(':festgelegteUserID', $festgelegteUserID);
                $query->execute();

                while ($zeile = $query->fetchObject()) {
                    echo "<a href='profil.php?user_id=$zeile->user_id'>$zeile->username</a><br>";
                }
                ?>
